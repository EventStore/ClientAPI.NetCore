...
namespace Eventstore.ClientAPI.Tests.Security
{
    ...
    public class stream_security_inheritance: AuthenticationTestBase
    {
        ...
        public override void OneTimeSetUp()
        {
            ...
            SetStreamMetadata("user-no-acl", ExpectedVersion.NoStream,
                              StreamMetadata.Build(), new UserCredentials("adm", "admpa$$"));
            SetStreamMetadata("user-w-diff", ExpectedVersion.NoStream,
                              StreamMetadata.Build().SetWriteRole("user2"), new UserCredentials("adm", "admpa$$"));
            SetStreamMetadata("user-w-multiple", ExpectedVersion.NoStream,
                              StreamMetadata.Build().SetWriteRoles(new[] {"user1", "user2"}), new UserCredentials("adm", "admpa$$"));
            SetStreamMetadata("user-w-restricted", ExpectedVersion.NoStream,
                              StreamMetadata.Build().SetWriteRoles(new string[0]), new UserCredentials("adm", "admpa$$"));
            SetStreamMetadata("user-w-all", ExpectedVersion.NoStream,
                              StreamMetadata.Build().SetWriteRole(SystemRoles.All), new UserCredentials("adm", "admpa$$"));

            SetStreamMetadata("user-r-restricted", ExpectedVersion.NoStream,
                             StreamMetadata.Build().SetReadRole("user1"), new UserCredentials("adm", "admpa$$"));

            SetStreamMetadata("$sys-no-acl", ExpectedVersion.NoStream,
                              StreamMetadata.Build(), new UserCredentials("adm", "admpa$$"));
            SetStreamMetadata("$sys-w-diff", ExpectedVersion.NoStream,
                              StreamMetadata.Build().SetWriteRole("user2"), new UserCredentials("adm", "admpa$$"));
            SetStreamMetadata("$sys-w-multiple", ExpectedVersion.NoStream,
                              StreamMetadata.Build().SetWriteRoles(new[] { "user1", "user2" }), new UserCredentials("adm", "admpa$$"));
            SetStreamMetadata("$sys-w-restricted", ExpectedVersion.NoStream,
                              StreamMetadata.Build().SetWriteRoles(new string[0]), new UserCredentials("adm", "admpa$$"));
            SetStreamMetadata("$sys-w-all", ExpectedVersion.NoStream,
                                         StreamMetadata.Build().SetWriteRole(SystemRoles.All), new UserCredentials("adm", "admpa$$"));
        }
